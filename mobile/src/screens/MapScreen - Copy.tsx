    ...
    <Animated.View style={[fs.sheet,
      ...
      >
        {CONNECTOR_TYPES.map(opt => {
          const isActive = JSON.stringify(filters.connectorType) ===
            JSON.stringify(opt.value);
          return (
            <TouchableOpacity key={opt.label}
              style={[fs.chip, isActive && fs.chipActive]}
              onPress={() => setFilters({ connectorType: opt.value })}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        ...
        {VEHICLE_TYPES.map(opt => (
          <TouchableOpacity key={opt.value}
            style={[fs.chip, filters.vehicleType === opt.value && fs.chipActive]}
            onPress={() => setFilters({ vehicleType: opt.value })}>
            <Text style={[fs.chipText,
              filters.vehicleType === opt.value && fs.chipTextActive]}>
              {opt.label}
            </Text>
          </TouchableOpacity>
        ))}
    ...
    (chargers || []).filter(c => {
      ...
      if (filters.minPower > 0) {
        const hasEnoughPower = (c.connectorStatuses || [])
          .some((conn: any) => {
            const kw = (conn.maxPowerWatts || c.maxPower || 0) / 1000;
            return kw >= filters.minPower;
          });
        // Fallback to charger-level if no per-connector data
      ...
      if (filters.vehicleType !== 'all') {
        ...
        if (hasVehicleData) {
          const supportsVehicle = connectors.some((conn: any) => {
            ...
            const cats = conn.vehicleCategory.split(',').map((v: string) => v.trim());
            return cats.includes(filters.vehicleType);
          });
